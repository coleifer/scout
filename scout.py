...
=====
  ...
  Returns the list of indexes and the number of documents contained
  within each.

  POSTing a `name` to this view will create a new index.


  Index detail: "/<index name>/"
  ------------------------------

  Return a paginated list of documents contained in the index.

  POSTing a `name` to this view will update the name of the index.

  DELETE requests will delete the index but none of the documents.


  Index search: "/<index names>/search/"
  --------------------------------------

  Return a paginated list of documents matching the provided search
  query. Search queries use the `q` GET parameter. You can also
  filter on metadata by passing arbitrary key/value GET pairs.
  ...
  POSTing `content`, `index` or `indexes` and optionally `metadat`
  will create a new document in the given index(es).


  Document detail: "/documents/<document id>/"
  --------------------------------------------

  Return document content, indexes and metadata.

  POST requests can be used to update the `content`, `index(es)`,
  and/or `metadata`.

  DELETE requests will delete the document.
"""
